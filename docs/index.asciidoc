...
----
* Snce v1.10.0 of {esf-name}, the field `_id` is not present anymore and the `@metadata._id` one is s now sent to Logstash: only setting `document_id` as in the sample configuration above is enough.
