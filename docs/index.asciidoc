...
When this input is used along with an {logstash-ref}/plugins-outputs-elasticsearch.html#plugins-outputs-elasticsearch[Elasticsearch output pluginh], you have to set the `document_id` in the output configuration.
...
* For {esf-name} v1.9.0 and below you have to rename the field `_id` to `@metadata._id` with a filter:
...
----
* Starting from version 1.10.0 of {esf-name}, the `_id` field is no longer available, and instead, Logstash now receives the `@metadata._id` field. Simply configuring `document_id` as shown in the example above is sufficient."
